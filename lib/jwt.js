...
function setToken (r, jwt_algo, cookie_name) {
  return (obj, key) => {
    r.session = obj;
...
function cookie_filter (r, jwt_algo, jwt_key, cookie_name, proxy) {
  let obj;
  if (r.sessionid) {
  ...
  r.setSessionToken = setToken(r, jwt_algo, cookie_name);
  r.getSessionToken = getToken(jwt_algo);
}
