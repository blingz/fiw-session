...
function session(conn, opts = {}) {
    ...
    //JSON Web Token
    let jwt_enabled = utils.jwt_enabled(opts);
    let jwt_sid     = utils.jwt_sid(opts);
    let jwt_verify  = utils.jwt_verify(opts);
    ...
    this.cookie_filter = (r) => {
        ...
        //jwt
        if (jwt_enabled && jwt_verify) {
            var jwt_token = null;
            var jwt_cookie = r.cookies[jwt_sid];
            if(jwt_cookie) {
                jwt_token = jws.decode(jwt_cookie);
            }
            r.setJsonWebToken = jwt(r, jwt_sid);
            jwt_verify(r, jwt_token);
        }
    };
